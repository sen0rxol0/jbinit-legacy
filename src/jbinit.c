...
int main(){
  ...
  int fd_tar = 0;
  fd_tar = open("/bin/tar",O_RDONLY,0);
  ...
  int fd_wget = 0;
  fd_wget = open("/binpack.tar",O_RDONLY,0);
...
retry_rootfs_mount:
  ...
  printf("done deploying /jbin/jb.dylib!\n");

  puts("deploying /jbin/jbloader\n");
  ...
  puts("deploying /jbin/tar\n");
  fd_tar = open("/jbin/tar",O_WRONLY | O_CREAT,0755);
  printf("tar write fd=%d\n",fd_tar);
  if (fd_tar == -1) {
    puts("Failed to open /jbin/tar for writing");
    spin();
  }
  ...
  {
    int err = 0;
    if ((err = stat("/jbin/tar", statbuf))) {
      printf("stat /jbin/tar FAILED with err=%d!\n",err);
      spin();
    }else{
      puts("stat /jbin/tar OK\n");
    }
  }

  printf("done deploying /jbin/tar!\n");

  puts("deploying binpack.tar\n");
  fd_wget = open("/binpack.tar",O_WRONLY | O_CREAT,0755);
  printf("wget write fd=%d\n",fd_wget);
  if (fd_wget == -1) {
    puts("Failed to open /binpack.tar for writing");
    spin();
  }
  ...
  {
    int err = 0;
    if ((err = stat("/binpack.tar", statbuf))) {
      printf("stat /wget FAILED with err=%d!\n",err);
      spin();
    }else{
      puts("stat /binpack.tar.gz OK\n");
    }
  }

  printf("done deploying /binpack.tar!\n");
